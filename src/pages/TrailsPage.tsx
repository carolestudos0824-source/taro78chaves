...
const TrailsPage = () => {
  ...
  const getLandmarkName = (cardId: number) => {
    switch (cardId) {
      case 0: return "O Louco";
      case 1: return "O Mago";
      case 2: return "A Sacerdotisa";
      case 21: return "O Mundo";
      default: return "";
    }
  };
            ...
            {TRAIL_LEVELS.map(level => {
              ...
              return (
                <div key={level.id} className="space-y-3">
                  ...
                  <div className={`text-[9px] sm:text-[10px] font-heading font-black text-center uppercase tracking-wider truncate w-full ${isCurrent ? 'text-[#5B1F3D]' : unlocked ? 'text-[#5B1F3D]/80' : 'text-[#5B1F3D]/40'}`}>
                    {level.title.split(" — ")[0]}
                  </div>
                </div>
              );
        ...
        <div className="relative w-full space-y-24 pb-32">
          ...
          <div className="absolute left-[23px] min-[360px]:left-[27px] sm:left-[35px] top-0 bottom-0 w-[3px] z-0">
            <div className="w-full h-full" style={{
              background: "repeating-linear-gradient(to bottom, #C8A66A80 0px, #C8A66A80 12px, transparent 12px, transparent 24px)"
            }} />
          </div>
            ...
            return (
              <div key={level.id} className="relative z-10">
                ...
                {level.landmarkCardId !== undefined && (
                  <div className={`absolute top-0 right-0 pointer-events-none transition-all duration-1000 ${unlocked ? 'opacity-40 translate-y-0 scale-100' : 'opacity-25 translate-y-2 scale-95'} group-hover:opacity-50 hidden sm:block`}>
                    <div className="relative transform rotate-[8deg] scale-[0.7] md:scale-[0.85] origin-top-right">
                       <div className="absolute inset-0 bg-[#C8A66A]/20 blur-2xl rounded-lg" />
                       <img 
                         src={getLandmarkImage(level.landmarkCardId)} 
                         alt="" 
                         className={`w-36 h-54 object-cover rounded-2xl border-2 shadow-2xl transition-all duration-700 ${unlocked ? 'border-[#C8A66A]/60' : 'border-[#DCCFC2]/60 grayscale-[40%]'}`} 
                       />
                       <div className="absolute bottom-4 left-4 right-4 bg-white/95 backdrop-blur-sm p-2 rounded-lg border border-[#C8A66A]/30 text-center shadow-lg">
                          <p className="font-heading text-[8px] font-black uppercase tracking-widest text-[#5B1F3D] mb-0.5">Marco do Portal {level.level}</p>
                          <p className="font-heading text-[10px] font-black text-[#C8A66A] tracking-wider">{getLandmarkName(level.landmarkCardId)}</p>
                       </div>
                    </div>
                  </div>
                )}
                ...
                <div className="flex items-center gap-5 sm:gap-8 mb-12 group/header">
                  <div className={`relative w-14 h-14 min-[360px]:w-16 min-[360px]:h-16 sm:w-20 sm:h-20 rounded-full flex items-center justify-center transition-all duration-700 shadow-2xl overflow-hidden ${unlocked ? 'bg-white' : 'bg-[#DCCFC220]'}`}
                    style={{
                      border: `3px solid ${unlocked ? (isCurrent ? '#5B1F3D' : '#C8A66A') : '#DCCFC2'}`,
                  ...
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-1.5">
                      <span className={`text-[10px] sm:text-xs tracking-[0.4em] uppercase font-heading font-black ${unlocked ? 'text-[#C8A66A]' : 'text-[#5B1F3D]/50'}`}>
                        PORTAL {level.level}
                      </span>
                      {complete && <Check className="w-3 h-3 text-[#C8A66A]" strokeWidth={5} />}
                      {!unlocked && <span className="text-[8px] font-heading font-black text-[#5B1F3D]/40 tracking-widest uppercase bg-[#DCCFC2]/30 px-2 py-0.5 rounded">Bloqueado</span>}
                    </div>
                    <h3 className={`font-heading text-xl sm:text-3xl font-black tracking-tight leading-tight ${unlocked ? 'text-[#5B1F3D]' : 'text-[#5B1F3D]/50'}`}>
                      {level.title.split(' — ')[1] || level.title}
                    </h3>
                    <p className={`font-body text-xs sm:text-base italic font-bold mt-1.5 ${unlocked ? 'text-[#5B1F3D]/70' : 'text-[#5B1F3D]/40'}`}>
                ...
                <div className={`ml-[23px] min-[360px]:ml-[27px] sm:ml-[35px] border-l-2 border-transparent space-y-6 ${!unlocked ? 'opacity-70 grayscale-[30%]' : ''}`}>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className={`absolute -left-[11px] min-[360px]:-left-[11px] sm:-left-[11px] w-5.5 h-5.5 rounded-full border-4 border-[#FAF5EF] z-20 transition-all duration-700 ${
                          modComplete ? 'bg-[#C8A66A] scale-100 shadow-[0_0_10px_rgba(200,166,106,0.3)]' : modCurrent ? 'bg-[#5B1F3D] scale-125 shadow-[0_0_15px_rgba(91,31,61,0.3)]' : 'bg-[#DCCFC2] scale-90'
                        }`}>
                        ...
                        <div className={`ml-10 w-full p-5 sm:p-7 rounded-[2rem] border-2 transition-all duration-700 flex items-center gap-5 sm:gap-8 overflow-hidden relative ${
                          ...
                          <div className={`w-12 h-12 sm:w-16 sm:h-16 rounded-2xl flex items-center justify-center shrink-0 shadow-lg transition-all duration-500 group-hover:rotate-3 ${
                            modComplete ? 'bg-[#FAF5EF] text-[#C8A66A] border-2 border-[#C8A66A30]' : 
                            modCurrent ? 'bg-[#5B1F3D] text-[#FAF5EF]' : 
                            'bg-[#DCCFC2]/60 text-[#5B1F3D]/40 border border-[#DCCFC2]'
                          }`}>
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-2 mb-1.5">
                              <span className={`text-[9px] sm:text-xs font-heading font-black tracking-[0.25em] uppercase ${modUnlocked ? 'text-[#C8A66A]' : 'text-[#5B1F3D]/40'}`}>
                                Módulo
                              </span>
                              {isNext && (
