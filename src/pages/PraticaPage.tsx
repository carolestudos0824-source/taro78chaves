...
const PraticaPage = () => {
  ...
  const completedCount = PRATICA_LESSONS.filter((l) =>
    isLessonCompleted(l.id)
  ).length;

  const progressPct = Math.round((completedCount / PRATICA_LESSONS.length) * 100);
    ...
    <div className="min-h-screen relative overflow-hidden">
      <div className="fixed inset-0 z-0 mystic-bg-procedural">
        ...
        <div className="absolute inset-0" style={{
          background: "linear-gradient(to bottom, hsl(270 30% 25% / 0.06) 0%, hsl(36 33% 97% / 0.05) 30%, hsl(36 33% 97% / 0.08) 70%, hsl(270 30% 25% / 0.12) 100%)"
        }} />
      </div>

      <header className="relative z-10" style={{
        borderBottom: "1px solid hsl(270 30% 25% / 0.25)",
        background: "linear-gradient(180deg, hsl(36 33% 96% / 0.94) 0%, hsl(38 28% 93% / 0.92) 100%)",
        backdropFilter: "blur(28px)",
        boxShadow: "0 6px 36px hsl(270 30% 25% / 0.08), 0 1px 0 hsl(270 30% 25% / 0.12) inset",
      }}>
        <div className="container max-w-3xl py-5 px-6">
          <div className="flex items-center gap-4 mb-4">
            ...
            <div className="flex flex-col flex-1">
              ...
              <h1 className="font-heading text-xl md:text-2xl tracking-wide" style={{
                background: "linear-gradient(135deg, hsl(270 30% 22%), hsl(270 25% 35%), hsl(36 45% 44%))",
          ...
          <div className="space-y-1.5">
            ...
            <div className="h-2 rounded-full overflow-hidden" style={{ background: "hsl(270 18% 88%)", border: "1px solid hsl(270 22% 78% / 0.50)" }}>
              <div className="h-full rounded-full transition-all duration-700" style={{
                width: `${progressPct}%`,
                background: "linear-gradient(90deg, hsl(270 30% 26%), hsl(270 25% 42%))",
              }} />
            </div>
          </div>
        </div>
      ...
      <main className="relative z-10 container max-w-3xl py-8 px-6">
        <div className="rounded-xl p-5 mb-8" style={{
          background: "linear-gradient(145deg, hsl(270 30% 25% / 0.04), hsl(36 45% 58% / 0.03))",
          border: "1px solid hsl(270 30% 25% / 0.15)",
        }}>
          <p className="font-accent text-sm italic leading-relaxed text-center" style={{ color: "hsl(230 20% 25% / 0.70)" }}>
            Aqui você aplica tudo o que aprendeu em <strong style={{ color: "hsl(270 30% 22%)" }}>leituras simuladas, correções comentadas e exercícios reais</strong>.
            Este módulo transforma conhecimento em habilidade.
          </p>
        </div>

        <div className="space-y-3">
          {PRATICA_LESSONS.map((lesson, i) => {
            ...
            return (
              <button
                key={lesson.id}
                onClick={() => unlocked && navigate(`/pratica/${lesson.order}`)}
                disabled={!unlocked}
              ...
              >
                <div className="relative overflow-hidden rounded-xl transition-all duration-400" style={isCurrent ? {
                  background: "linear-gradient(145deg, hsl(38 28% 93% / 0.94), hsl(270 20% 95% / 0.90))",
                  backdropFilter: "blur(18px)",
                  border: "1.5px solid hsl(270 30% 25% / 0.35)",
                  boxShadow: "0 6px 28px hsl(270 30% 25% / 0.10), 0 0 40px hsl(270 30% 25% / 0.04)",
                } : completed ? {
                  background: "hsl(38 28% 94% / 0.80)",
                  backdropFilter: "blur(12px)",
                  border: "1px solid hsl(270 25% 45% / 0.25)",
                  boxShadow: "0 3px 14px hsl(270 30% 25% / 0.05)",
                } : {
                  ...
                  <div className="p-5 flex items-center gap-4">
                    <div className="w-11 h-11 rounded-full flex items-center justify-center shrink-0 transition-all duration-500" style={isCurrent ? {
                      border: "2px solid hsl(270 30% 26% / 0.45)",
                      background: "linear-gradient(135deg, hsl(38 28% 93%), hsl(270 30% 25% / 0.10))",
                      boxShadow: "0 0 20px hsl(270 30% 25% / 0.12)",
                    } : completed ? {
                      border: "2px solid hsl(270 25% 45% / 0.35)",
                      ...
                      {completed ? (
                        <Check className="w-5 h-5" style={{ color: "hsl(270 25% 38%)" }} />
                      ) : unlocked ? (
                    ...
                    {unlocked && (
                      <ChevronRight className="w-4 h-4 shrink-0 transition-transform group-hover:translate-x-1" style={{
                        color: isCurrent ? "hsl(270 30% 25% / 0.50)" : "hsl(270 25% 45% / 0.40)",
                      }} />
                    )}
                  </div>
                </div>
        ...
        {completedCount === PRATICA_LESSONS.length && (
          <div className="mt-8 rounded-xl p-6 text-center" style={{
            background: "linear-gradient(135deg, hsl(270 30% 25% / 0.06), hsl(36 45% 58% / 0.04))",
            border: "1px solid hsl(270 30% 25% / 0.20)",
            animation: "fade-up 0.6s ease-out",
          }}>
            <div className="text-2xl mb-3">🏆</div>
            <h3 className="font-heading text-lg tracking-wide mb-2" style={{ color: "hsl(270 30% 22%)" }}>
